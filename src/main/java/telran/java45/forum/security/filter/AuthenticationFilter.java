...
public class AuthenticationFilter implements Filter {
	...
	final UserAccountRepository userEcountRepository;
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (checkEndPoint(request.getMethod(), request.getServletPath())) {
			...
			try {
				...
				response.sendError(401, "token is not valod");

			}
			UserAccount userAccount = userEcountRepository.findById(credentials[0]).orElse(null);
			if (userAccount == null || !BCrypt.checkpw(credentials[1], userAccount.getPassword())) {
				response.sendError(401, "login or password not valod");
				return;
			} 
